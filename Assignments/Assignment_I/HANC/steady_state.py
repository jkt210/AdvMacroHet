...
def prepare_hh_ss(model):
    ...
    # d. phi
    par.phi_grid[:] = np.array([par.phi_low,par.phi_low,par.phi_low,par.phi_high,par.phi_high,par.phi_high])

    # e. eta
    par.eta_grid[:] = np.array([par.eta_0,par.eta_0,par.eta_0,par.eta_1,par.eta_1,par.eta_1])
    ...
    ss.Dbeg[0,:,0] = z_ergodic*par.chi_low_prob*1/par.Nbeta
    ss.Dbeg[1,:,0] = z_ergodic*par.chi_low_prob*1/par.Nbeta
    ss.Dbeg[2,:,0] = z_ergodic*par.chi_low_prob*1/par.Nbeta
    ss.Dbeg[3,:,0] = z_ergodic*par.chi_high_prob*1/par.Nbeta
    ss.Dbeg[4,:,0] = z_ergodic*par.chi_high_prob*1/par.Nbeta
    ss.Dbeg[5,:,0] = z_ergodic*par.chi_high_prob*1/par.Nbeta
    ...
    # a. raw value
    y = (ss.w0*par.phi_low + ss.w1*par.phi_high)*par.z_grid
...
def obj_ss(K_ss,model,do_print=False):
    ...
    # b. implied prices
    ss.rK = par.alpha*ss.Gamma*(ss.K)**(par.alpha-1.0)*ss.L0**((1.0-par.alpha)/2.0)*ss.L1**((1.0-par.alpha)/2.0)
    ss.r = ss.rK - par.delta
    ss.w0 = ((1.0-par.alpha)/2.0)*ss.Gamma*(ss.K)**par.alpha*ss.L0**((-1.0-par.alpha)/2.0)*ss.L1**((1.0-par.alpha)/2.0)
    ss.w1 = ((1.0-par.alpha)/2.0)*ss.Gamma*(ss.K)**par.alpha*ss.L0**((1.0-par.alpha)/2.0)*ss.L1**((-1.0-par.alpha)/2.0)
